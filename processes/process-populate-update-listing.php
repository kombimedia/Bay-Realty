...
// // Define variables to save dynamic form elements into
// $options_agents = "";

// $stmt = $mysqli->prepare("SELECT properties.listing_id, properties.agents, properties.title, properties.address, properties.categories, properties.type, properties.price, properties.sell_method, properties.property_des, properties.bed_no, properties.bed_des, properties.bath_no, properties.bath_des, properties.lounge_no, properties.lounge_des, properties.garage_no, properties.garage_des, properties.house_size, properties.land_size, properties.map_co_ords, properties.featured_image, properties.featured_property, categories.cat_id, categories.city, property_type.pt_id, property_type.type, agents.agent_id, agents.first_name, agents.surname FROM properties INNER JOIN categories ON properties.categories = categories.cat_id INNER JOIN property_type ON properties.type = property_type.pt_id INNER JOIN agents ON properties.agents = agents.agent_id WHERE listing_id = ?");
// $stmt->bind_param("i", $listing_id);
// $stmt->execute();
// $result = $stmt->get_result();
// if($result->num_rows > 0) {
...
//       // Dynamically build each 'Select List' list with the saved option preselected

//       // $arr[] = $row;
//       // $agent_name = $row['first_name'] . " " . $row['surname'];
//       //   // compare the stored value from the properties table to that of the agents table. If there is a match add the 'selected' tag to that option
//       // if ($agents === $row['agent_id']) {
//       //     $options_agents = $options_agents . "<option selected value='$row[agent_id]'>$agent_name</option>";
//       //       // if the stored value does not match the value in the agents table create an option with no 'selected' tag
//       //     } else {
//       //       $options_agents = $options_agents . "<option value='$row[agent_id]'>$agent_name</option>";
//       //       }
